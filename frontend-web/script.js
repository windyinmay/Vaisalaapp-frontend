document.addEventListener('DOMContentLoaded', function () {
	...
	const calibrationOptionsPage = document.getElementById(
	...
	const calibrationInputPage = document.getElementById(
		'calibration-input-page'
	);
	const calibrationInputBackBtn = document.getElementById(
		'calibration-input-back-btn'
	);
	const calibrationInputNextBtn = document.getElementById(
		'calibration-input-next-btn'
	);
	...
	const maintenanceOptions = document.querySelectorAll(
		'input[name="maintenance-option"]'
	);
	const maintenanceIntervalPage = document.getElementById(
	...
	const maintenanceCompletePage = document.getElementById(
	...
	const instructionsToolbarBtn = document.querySelector(
		'.toolbar-item:nth-child(2)'
	); // Instructions in toolbar
	const installationGuidePage = document.getElementById(
		'installation-guide-page'
	);
	const installationGuideBackBtn = document.getElementById(
		'installation-guide-back-btn'
	);
	const installationGuideNextBtn = document.getElementById(
		'installation-guide-next-btn'
	);

	const probeMountingPage = document.getElementById('probe-mounting-page');
	const probeMountingBackBtn = document.getElementById(
		'probe-mounting-back-btn'
	);
	const probeMountingNextBtn = document.getElementById(
		'probe-mounting-next-btn'
	);
	...
	const usernameInput = document.getElementById('username');
	//const loginForm = document.getElementById('login-form');
	const setupMaintenanceOption = document.getElementById(
		'setup-maintenance-option'
	);
	const skipMaintenanceOption = document.getElementById(
		'skip-maintenance-option'
	);
		...
		if (backButton) {
			...
			if (backButtonText) {
				backButtonText.textContent =
					flow === 'setup' ? 'Set up new sensor' : 'Calibrate existing sensor';
			}
		}
	...
	if (mainMenuBtns) {
		mainMenuBtns.forEach((btn) => {
			btn.addEventListener('click', function () {
				...
				if (this.id === 'mainmenu1') {
					// "Set up new sensor"
					showPage('app-overview-page');
				} else if (this.id === 'mainmenu2') {
					// "Calibrate existing sensor"
					showPage('app-overview-page');
				} else if (this.id === 'mainmenu3') {
					// "Instructions"
	...
	function updateProgressBar(pageId) {
		...
		if (progressBarPages.includes(pageId)) {
			...
			if (pageId === 'configuration-page') {
				...
				document
					.getElementById('step-calibrate')
					.classList.remove('current', 'completed');
				document
					.getElementById('step-maintain')
					.classList.remove('completed', 'current');
	...
	if (calibOptionsNextBtn) {
		calibOptionsNextBtn.addEventListener('click', function () {
			...
			const selectedOption = document.querySelector(
				'input[name="calibration-option"]:checked'
			);
			if (selectedOption) {
	...
	function validateFieldOnBlur(input, errorMessage) {
		...
		if (!validateInputFormat(input)) {
			...
			if (
				messageContainer &&
				(validateInputFormat(temperatureInput) ||
					temperatureInput.value.trim() === '') &&
				(validateInputFormat(humidityInput) ||
	...
	if (document.getElementById('calibrate-option')) {
		document
			.getElementById('calibrate-option')
	...
	document.addEventListener('DOMContentLoaded', function () {
		// Initial state setup
		document.getElementById('calibration-due-date').textContent =
			'-- Select an interval --';
	...
	const instructionsToolbarItem = document.querySelector(
		'.toolbar-item:nth-child(2)'
	);
	...
	const assistanceToolbarItem = document.querySelector(
		'.toolbar-item:nth-child(3)'
	);
	const settingsToolbarItem = document.querySelector(
	...
	if (historyLogElement && historyLogContentElement) {
		...
		if (seeMoreSpan) {
			seeMoreSpan.addEventListener('click', function () {
				...
				this.textContent = historyLogContentElement.classList.contains('hidden')
					? 'See more'
					: 'See less';
			});
		}
