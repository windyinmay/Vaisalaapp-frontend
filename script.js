document.addEventListener('DOMContentLoaded', function () {
    ...
    const DISCONNECT_TIMEOUT = 2000; // 2 seconds without a reading is considered disconnected
    ...
    async function startNFC() {
        ...
        try {
            ...
            connectionCheckInterval = setInterval(() => {
                ...
                // If we haven't had a reading in 1 second, increment the missed readings counter
                if (lastReadingTime > 0 && (currentTime - lastReadingTime > 1000)) {
                    missedReadingsCount++;
                }

                // If we were connected and have missed too many readings
                if (isConnected && missedReadingsCount >= DISCONNECT_THRESHOLD) {
                    isConnected = false;
                    if (nfcStatus) {
            ...
            ndef.addEventListener("reading", (event) => {
                ...
                // Reset the missed readings counter
                missedReadingsCount = 0;
    ...
    // Function to clean up when navigating away or stopping
    function stopNFC() {
        if (connectionCheckInterval) {
            clearInterval(connectionCheckInterval);
            connectionCheckInterval = null;
        ...
        if (nfcStatus) {
            nfcStatus.textContent = 'NFC scanning stopped';
        }
    }
    async function startNFC13.10() {
        // Check if Web NFC is available
        if (typeof NDEFReader === 'undefined') {
            if (nfcStatus) {
                nfcStatus.textContent = 'Web NFC is not supported in this browser';
